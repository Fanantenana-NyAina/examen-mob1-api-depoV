...
export class LabelServices {
  ...
  static async getOneById(accountId: string, id: string) {
    const getLabelById = await getPrismaClient().label.findFirst({
      where: { id, accountId },
  ...
  static async archiveOneById(accountId: string, id: string) {
    ...
    return await getPrismaClient().label.update({
      data: { isArchived: !getLabelById.isArchived },
      where: { id, accountId },
    });
  }

  static async getAll(accountId: string, query: ListFilters & NameFilter & { isArchived?: string | boolean }) {
    const { page, pageSize, name = "", isArchived } = query;

    const isArchivedBool = isArchived === true || isArchived === "true" ? true : false;
    const where = { accountId, name: { contains: name }, isArchived: isArchivedBool };
